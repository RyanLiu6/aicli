...
1. First, check the current state:
   ...
   - Check the current branch - NEVER commit directly to main/master
   - If on main/master, ask the user what branch to use (you can suggest a descriptive branch name)
...
- **Do not push commits** - this skill only creates commits locally. Use `/create-pr` or `/update-pr` for pushing
- Always respect the user's git workflow rules from memory/base.md