...
def _collect_skill_files(repo_root: Path) -> list[tuple[str, Path]]:
    skills = find_skill_files(repo_root / "skills")
    work_skills = repo_root / "work" / "skills"
    if work_skills.exists():
        skills.extend(find_skill_files(work_skills))
    return skills


def _get_skill_ids(repo_root: Path) -> list[str]:
    return [name for name, _ in _collect_skill_files(repo_root)]
